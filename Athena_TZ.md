...
> **Версия:** 1.3 | **Дата:** Апрель 2026 | **Статус:** Актуально (`v2-revival` как активная параллельная ветка, `v3` — отдельно)
...
| v2 (исследовательский цикл) | Hybrid Signal + Sentiment + RL Shield + Streamlit | Исследован и законсервирован как baseline |
| v2.5 (salvage) | `core_compact` + short-only + meta-filter + diagnostics | Архив негативного контроля |
| v2-revival | Regression-driven восстановление `v2` в отдельной ветке | Активный track |
| v3 (next) | Regime-first + Session-aware + rolling retrain | Отдельный parallel track |
...
## 11. Переход на Athena v3: Regime-first + Session-aware + rolling retrain

### 11.1 Почему меняется архитектура

По итогам строгого OOS-цикла для 15m стало ясно:

- универсальная модель по смешанным рыночным режимам недостаточно устойчива;
- `short-only` retrain улучшил сырой edge, но не сделал торговую систему устойчиво прибыльной;
- `meta-filter` дал локально хорошие validation-метрики, но на честном holdout не спас ветку `v2`.

Следовательно, новый фокус смещается с «одной общей модели на всё» к архитектуре:

1. сначала определить **режим рынка** (`quiet / normal / hot`),
2. затем определить **контекст времени / торговой сессии** (`asia / europe / us / overlap / weekend`),
3. и только после этого выбирать конкретную торговую подстратегию.

### 11.2 Новый базовый принцип

- `v2` сохраняется как **архивный baseline** и негативный контроль;
- `v3` строится как **router-first system**, а не как одна “магическая” ML-модель;
- переобучение должно быть **rolling** и коротким по окнам, а не редким и общим для всех состояний рынка.

### 11.3 Целевая схема Athena v3

- `SessionContext` — извлекает временной контекст и bucket торговой сессии.
- `RegimeRouter` — определяет маршрут (`no_trade`, `mean_reversion`, `directional`, `breakout`).
- Подстратегии работают только внутри релевантного режима/сессии.
- Walk-forward переносится на более короткие циклы:
  - `train`: 30–45 дней,
  - `validation`: 7 дней,
  - `test`: следующие 7 дней,
  - обновление — раз в неделю или по drift-trigger.

### 11.4 Обязательные правила внедрения

1. Любая новая ветка начинается с **TDD**.
2. После каждого этапа — **unit + regression + strict walk-forward**.
3. Если ветка не проходит honest OOS, она не «допиливается бесконечно», а фиксируется как контроль и архивируется.
4. Все изменения и ответвления фиксируются в:
   - `change_history.md`
...
- Архив `v2` сохранён в `backups/strategy_archive_2026-04-08/`.
- Основная рабочая программа переносится в документ:
  - `docs/ATHENA_V3_REGIME_FIRST_PLAN_AND_TZ_RU.md`

*⚡ Athena AI-Bot v2/v3 transition | Техническое задание v1.2 | Апрель 2026*
