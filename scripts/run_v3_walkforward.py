#!/usr/bin/env python3
"""Launch Athena v3 regime-first walk-forward runs with isolated artifacts."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Athena v3 walk-forward experiments with isolated outputs")
    ...
    parser.add_argument("--direction", default="short", choices=["both", "long", "short"])
    ...
    parser.add_argument("--suffix", default="regime_first", help="Label appended to the result filename")
...
def main() -> None:
    ...
    output_json = Path(args.output_json) if args.output_json else default_result_path(
        "v3", timeframe=args.timeframe, candidate=args.candidate, suffix=args.suffix
    )
    model_dir = track_dir("v3", "models")
